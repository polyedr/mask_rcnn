...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 form_login.py train --dataset=/path/to/form_login/dataset --weights=coco

    # Resume training a model that you had trained earlier
    python3 form_login.py train --dataset=/path/to/form_login/dataset --weights=last

    # Train a new model starting from ImageNet weights
    python3 form_login.py train --dataset=/path/to/form_login/dataset --weights=imagenet

    # Apply color splash to an image
    python3 form_login.py splash --weights=/path/to/weights/file.h5 --image=<URL or path to file>

    # Apply color splash to video using the last weights you trained
    python3 form_login.py splash --weights=last --video=<URL or path to file>
"""
...
def train(model):
    ...
    dataset_train.load_form_login(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = Form_loginDataset()
    dataset_val.load_form_login(args.dataset, "val")
    dataset_val.prepare()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect form_logins.')
    parser.add_argument("command",
                        metavar="<command>",
                        help="'train' or 'splash'")
    parser.add_argument('--dataset', required=False,
                        metavar="/path/to/form_login/dataset/",
                        help='Directory of the Form_login dataset')
    ...
    # Configurations
    if args.command == "train":
        config = Form_loginConfig()
    else:
        class InferenceConfig(Form_loginConfig):
